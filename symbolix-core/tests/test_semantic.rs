...
fn analyzer_reuses_registered_variables_from_context() {
    new_compile_context! {
        let _ = symbolix_core::semantic::variable::Variable::new("shared", VariableType::Boolean, None);
...
fn analyzer_panics_on_invalid_manual_expressions() {
    new_compile_context! {
        ...
        let invalid_ternary = Expression::ternary(
            Expression::constant(Constant::Boolean(true)),
            Symbol::Other(symbolix_core::lexer::symbol::Other::Comma),
        ...
        let invalid_unary_symbol = Expression::unary(
            Symbol::Other(symbolix_core::lexer::symbol::Other::Semicolon),
            Expression::constant(Constant::integer(1)),
        );
