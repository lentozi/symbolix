...
pub fn convert_block(
        ...
        match stmt {
            ...
            syn::Stmt::Local(local) => {
                ...
                let var_name = match &pat {
                    ...
                    _ => {
                        return Err(syn::Error::new_spanned(
                            &pat,
                            "exprion! only supports identifier bindings in let statements",
                        ))
                    }
                };
                ...
                let expr_token = local.init.as_ref().ok_or_else(|| {
                    syn::Error::new_spanned(local, "exprion! requires let bindings to have initializers")
                })?;
            ...
            syn::Stmt::Expr(expr, semi) => {
                if semi.is_some() {
                    return Err(syn::Error::new_spanned(
                        expr,
                        "exprion! block must end with an expression, not a statement",
                    ));
                }
    ...
    Err(syn::Error::new(
        block.span(),
        "exprion! block must end with a return expression or tuple",
    ))
}
    ...
    match expr {
        Expr::Lit(lit_expr) => Ok(CompileValue::Semantic(match &lit_expr.lit {
            ...
            _ => {
                return Err(syn::Error::new_spanned(
                    lit_expr,
                    "exprion! only supports integer, float, and bool literals",
                ))
            }
        })),
        Expr::Binary(binary_expr) => {
            let left = expect_semantic(
                convert_expr(binary_expr.left.as_ref(), table)?,
                binary_expr.left.as_ref(),
            )?;
            let right = expect_semantic(
                convert_expr(binary_expr.right.as_ref(), table)?,
                ...
                _ => {
                    let left = match left {
                        ...
                        _ => {
                            return Err(syn::Error::new_spanned(
                                &binary_expr.left,
                                "comparison operators in exprion! require numeric expressions",
                            ))
                        }
                    };
                    ...
                    match &binary_expr.op {
                        BinOp::Eq(_) => Ok(CompileValue::Semantic(SemanticExpression::logical(
                        ...
                        BinOp::Ne(_) => Ok(CompileValue::Semantic(SemanticExpression::logical(
                            LogicalExpression::relation(
                                &left,
                        ...
                        BinOp::Gt(_) => Ok(CompileValue::Semantic(SemanticExpression::logical(
                            LogicalExpression::relation(
                                &left,
                        ...
                        BinOp::Lt(_) => Ok(CompileValue::Semantic(SemanticExpression::logical(
                            LogicalExpression::relation(
                                &left,
                        ...
                        BinOp::Ge(_) => Ok(CompileValue::Semantic(SemanticExpression::logical(
                            LogicalExpression::relation(
                                &left,
                        ...
                        BinOp::Le(_) => Ok(CompileValue::Semantic(SemanticExpression::logical(
                            LogicalExpression::relation(
                                &left,
                        ...
                        _ => Err(syn::Error::new_spanned(
                            &binary_expr.op,
                            "exprion! encountered an unsupported binary operator",
                        )),
                    }
                }
            }
            ...
            if let Some(ident) = mac.path.get_ident() {
                match ident.to_string().as_str() {
                    ...
                    "expr" => {
                        ...
                        let expression = exprion_core::parser::Parser::pratt(&mut lexer);
                        let mut analyzer = Analyzer::new();
                        Ok(CompileValue::Semantic(
                            analyzer.analyze_with_ctx(&expression),
                        ))
                    }
                    "solve" => {
                        let args: SolveArgs = syn::parse2(tokens)?;
                        if args.args.is_empty() || args.args.len() > 2 {
                            return Err(syn::Error::new_spanned(
                                macro_call,
                                "solve!(equation) or solve!(equation, variable) expects 1 or 2 arguments",
                            ));
                        }
                        ...
                        let equation_name = match equation_arg {
                            ...
                            _ => return Err(syn::Error::new_spanned(
                                equation_arg,
                                "solve!(equation, ...) requires `equation` to be a named binding",
                            )),
                        };
                        ...
                        let equation_ir = table.get(&equation_name).ok_or_else(|| {
                            syn::Error::new_spanned(
                                equation_arg,
                                format!("undefined binding `{equation_name}` in exprion! block"),
                            )
                        })?;
                        ...
                        let equation = if args.args.len() == 2 {
                            ...
                            let solve_name = match solve_arg {
                                ...
                                _ => {
                                    return Err(syn::Error::new_spanned(
                                        solve_arg,
                                        "solve!(equation, variable) requires `variable` to be a named binding",
                                    ))
                                }
                            };
                            let solve_ir = table.get(&solve_name).ok_or_else(|| {
                                syn::Error::new_spanned(
                                    solve_arg,
                                    format!("undefined binding `{solve_name}` in exprion! block"),
                                )
                            })?;
                            let solve_for = match solve_ir {
                                ...
                                _ => {
                                    return Err(syn::Error::new_spanned(
                                        solve_arg,
                                        "solve!(equation, variable) requires `variable` to be a numeric variable binding",
                                    ))
                                }
                            };

                            exprion_core::equation::Equation::new(
                                expect_semantic(equation_ir.clone(), equation_arg)?,
                                solve_for,
                            )
                            .map_err(|err| syn::Error::new_spanned(equation_arg, err.to_string()))?
                        } else {
                            exprion_core::equation::Equation::infer(expect_semantic(
                                equation_ir.clone(),
                                equation_arg,
                            )?)
                            .map_err(|err| syn::Error::new_spanned(equation_arg, err.to_string()))?
                    ...
                    _ => Err(syn::Error::new_spanned(
                        macro_call,
                        format!("unsupported macro `{}` inside exprion!", ident),
                    )),
                }
            } else {
                Err(syn::Error::new_spanned(
                    macro_call,
                    "exprion! encountered an unsupported macro call",
                ))
            }
        }
        Expr::MethodCall(method_call_expr) => {
            ...
            let receiver_name = match receiver {
                ...
                _ => {
                    return Err(syn::Error::new_spanned(
                        receiver,
                        "method receiver in exprion! must be a named binding",
                    ))
                }
            };
            ...
            let receiver_ir = table.get(&receiver_name).ok_or_else(|| {
                syn::Error::new_spanned(
                    receiver,
                    format!("undefined binding `{receiver_name}` in exprion! block"),
                )
            })?;
            ...
            if is_equation(receiver_ir) {
                ...
                if args.len() > 1 {
                    return Err(syn::Error::new_spanned(
                        method_call_expr,
                        format!(
                            "equation method `{}` in exprion! expects 0 or 1 argument, got {}",
                            method_name,
                            args.len()
                        ),
                    ));
                ...
                match method_name.as_str() {
                    "solve" => Err(syn::Error::new_spanned(
                        method_call_expr,
                        "use solve!(equation) or solve!(equation, variable) inside exprion!, not equation.solve(...)",
                    )),
                    _ => Err(syn::Error::new_spanned(
                        method_call_expr,
                        format!("unsupported equation method `{}` in exprion!", method_name),
                    )),
                }
            } else {
                if args.len() != 1 {
                    return Err(syn::Error::new_spanned(
                        method_call_expr,
                        format!(
                            "method `{}` in exprion! expects exactly 1 argument, got {}",
                            method_name,
                            args.len()
                        ),
                    ));
                ...
                let arg = args.first().ok_or_else(|| {
                    syn::Error::new_spanned(
                        method_call_expr,
                        format!("method `{}` in exprion! requires exactly 1 argument", method_name),
                    )
                })?;
                ...
                let arg_name: String = match arg {
                    ...
                    _ => {
                        return Err(syn::Error::new_spanned(
                            arg,
                            format!(
                                "method `{}` in exprion! requires a named variable argument",
                                method_name
                            ),
                        ))
                    }
                ...
                let arg_ir = expect_semantic(
                    table
                        ...
                        .ok_or_else(|| {
                            syn::Error::new_spanned(
                                arg,
                                format!("undefined binding `{arg_name}` in exprion! block"),
                            )
                        })?
                        .clone(),
                    arg,
                ...
                match (expect_semantic(receiver_ir.clone(), receiver)?, arg_ir) {
                    (SemanticExpression::Numeric(receiver), SemanticExpression::Numeric(arg)) => {
                        match method_name.as_str() {
                            "equal_to" => Ok(CompileValue::Semantic(SemanticExpression::logical(
                                LogicalExpression::relation(
                                    &receiver,
                                    &Symbol::Relation(Relation::Equal),
                                    &arg,
                            ...
                            "not_equal_to" => Ok(CompileValue::Semantic(
                            ...
                            "less_than" => Ok(CompileValue::Semantic(SemanticExpression::logical(
                                LogicalExpression::relation(
                                    &receiver,
                                    &Symbol::Relation(Relation::LessThan),
                                    &arg,
                            ...
                            "greater_than" => Ok(CompileValue::Semantic(
                                SemanticExpression::logical(LogicalExpression::relation(
                                    &receiver,
                                    &Symbol::Relation(Relation::GreaterThan),
                                    &arg,
                            ...
                            "less_equal" => Ok(CompileValue::Semantic(
                                SemanticExpression::logical(LogicalExpression::relation(
                                    &receiver,
                                    &Symbol::Relation(Relation::LessEqual),
                                    &arg,
                            ...
                            "greater_equal" => Ok(CompileValue::Semantic(
                                SemanticExpression::logical(LogicalExpression::relation(
                                    &receiver,
                                    &Symbol::Relation(Relation::GreaterEqual),
                                    &arg,
                            ...
                            _ => Err(syn::Error::new_spanned(
                                method_call_expr,
                                format!(
                                    "unsupported numeric method `{}` in exprion!",
                                    method_name
                                ),
                            )),
                        }
                    ...
                    _ => Err(syn::Error::new_spanned(
                        method_call_expr,
                        format!(
                            "method `{}` in exprion! is only supported for numeric expressions",
                            method_name
                        ),
                    )),
                }
        ...
        Expr::Path(variable) => {
            if let Some(ident) = variable.path.get_ident() {
                if let Some(semantic_ir) = table.get(&ident.to_string()) {
                    ...
                    Err(syn::Error::new_spanned(
                        variable,
                        format!("undefined binding `{}` in exprion! block", ident),
                    ))
                }
            } else {
                Err(syn::Error::new_spanned(
                    variable,
                    "exprion! encountered an unsupported path expression",
                ))
            }
        }
        Expr::Unary(unary_expr) => {
            let expr = expect_semantic(
                convert_expr(unary_expr.expr.as_ref(), table)?,
                unary_expr.expr.as_ref(),
            )?;
            match unary_expr.op {
                ...
                _ => Err(syn::Error::new_spanned(
                    unary_expr,
                    "exprion! encountered an unsupported unary operator",
                )),
            }
        }
        _ => Err(syn::Error::new_spanned(
            expr,
            "exprion! encountered an unsupported expression shape",
        )),
    }
}
...
fn handle_if(
    ...
    let cond = match expect_semantic(
        convert_expr(if_expr.cond.as_ref(), table)?,
        if_expr.cond.as_ref(),
    )? {
        ...
        _ => {
            return Err(syn::Error::new_spanned(
                &if_expr.cond,
                "if condition inside exprion! must be a logical expression",
            ))
        }
    };
    ...
    let expr = match expect_semantic(
        ...
        _ => {
            return Err(syn::Error::new_spanned(
                &if_expr.then_branch,
                "if branch inside exprion! must produce a numeric expression",
            ))
        }
    };
    ...
    let else_expr = if let Some(else_branch) = &if_expr.else_branch {
        match else_branch.1.as_ref() {
            ...
            _ => {
                return Err(syn::Error::new_spanned(
                    ...
                    "else branch inside exprion! must be a block or nested if",
                ))
            }
        }
    } else {
        return Err(syn::Error::new_spanned(
            if_expr,
            "exprion! requires an else branch for every if expression",
        ));
    };
    ...
    let else_expr = match expect_semantic(else_expr?, if_expr)? {
        ...
        _ => {
            return Err(syn::Error::new_spanned(
                if_expr,
                "else branch inside exprion! must produce a numeric expression",
            ))
        }
    };
...
fn handle_block_in_if(
    ...
    if expr_list.len() > 1 {
        return Err(syn::Error::new_spanned(
            block,
            "if blocks inside exprion! cannot return tuples",
        ));
    } else if expr_list.len() == 0 {
        return Err(syn::Error::new_spanned(
            block,
            "if blocks inside exprion! must end with a return expression",
        ));
    }
...
fn expect_semantic(value: CompileValue, span: &impl Spanned) -> syn::Result<SemanticExpression> {
    match value {
        ...
        CompileValue::SolutionSet(_) => Err(syn::Error::new(
            span.span(),
            "solution sets cannot participate in expression arithmetic inside exprion!",
        )),
    }
}
