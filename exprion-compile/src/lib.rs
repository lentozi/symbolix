...
use crate::expand::{
    compile_formula, compile_exprion, normalize_formula_input, normalize_exprion_input,
...
#[proc_macro]
pub fn exprion(input: TokenStream) -> TokenStream {
    match catch_unwind(AssertUnwindSafe(|| {
        let normalized_input = normalize_exprion_input(input.clone());
        expand_with_cache("exprion", &normalized_input, || compile_exprion(input))
            .unwrap_or_else(|err| err.to_compile_error().into())
    })) {
        Ok(tokens) => tokens,
        Err(payload) => panic_to_compile_error(payload),
