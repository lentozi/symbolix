...
const CACHE_DIR_ENV: &str = "EXPRION_CACHE_DIR";
const DISABLE_CACHE_ENV: &str = "EXPRION_DISABLE_CACHE";
const SILENCE_WARNING_ENV: &str = "EXPRION_SILENCE_CACHE_WARNING";
...
fn warn_on_fallback_cache_dir() {
    ...
    FALLBACK_WARNING.call_once(|| {
        ...
        eprintln!(
            "{prefix} exprion-compile is using the system cache directory because {CACHE_DIR_ENV} is not set; \
...
fn system_cache_root() -> PathBuf {
    ...
    std::env::temp_dir().join("exprion-macro-cache")
}
...
mod tests {
    ...
    fn temp_dir() -> PathBuf {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("exprion_compile_cache_unit_{unique}"))
    }

    #[test]
    fn stable_hash_is_deterministic() {
        assert_eq!(stable_hash("exprion"), stable_hash("exprion"));
        assert_ne!(stable_hash("exprion"), stable_hash("formula"));
    }

    #[test]
    fn env_flag_enabled_accepts_expected_values() {
        for value in ["1", "true", "TRUE", "yes", "YES", "on", "ON"] {
            std::env::set_var("EXPRION_TEST_FLAG", value);
            assert!(env_flag_enabled("EXPRION_TEST_FLAG"));
        }
        std::env::set_var("EXPRION_TEST_FLAG", "0");
        assert!(!env_flag_enabled("EXPRION_TEST_FLAG"));
        std::env::remove_var("EXPRION_TEST_FLAG");
    }
