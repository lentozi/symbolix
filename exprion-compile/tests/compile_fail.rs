...
fn make_temp_crate_dir() -> PathBuf {
    let unique = SystemTime::now()
        ...
        .as_nanos();
    std::env::temp_dir().join(format!("exprion_compile_fail_{unique}"))
}
...
fn write_temp_crate(main_rs: &str) -> PathBuf {
    ...
    fs::write(
        ...
        format!(
            r#"[package]
name = "exprion-compile-fail-case"
version = "0.1.0"
edition = "2021"
...
#[test]
fn exprion_reports_errors_inside_macro_body() {
    let temp_dir = write_temp_crate(
        r#"use exprion_compile::exprion;
...
"#,
    ...
    assert!(
        stderr.contains("undefined binding `missing` in exprion! block"),
...
#[test]
fn exprion_reports_missing_else_branch() {
    let temp_dir = write_temp_crate(
        r#"use exprion_compile::exprion;
...
#[test]
fn exprion_reports_unsupported_equation_method_call() {
    let temp_dir = write_temp_crate(
        r#"use exprion_compile::exprion;
...
#[test]
fn exprion_reports_non_identifier_let_pattern() {
    let temp_dir = write_temp_crate(
        r#"use exprion_compile::exprion;
...
#[test]
fn exprion_reports_invalid_var_type() {
    let temp_dir = write_temp_crate(
        r#"use exprion_compile::exprion;
...
#[test]
fn exprion_reports_method_receiver_must_be_named_binding() {
    let temp_dir = write_temp_crate(
        r#"use exprion_compile::exprion;
...
"#,
    ...
    let stderr = cargo_check_stderr(&temp_dir);
    assert!(stderr.contains("method receiver in exprion! must be a named binding"), "stderr:\n{stderr}");
    let _ = fs::remove_dir_all(&temp_dir);
}

#[test]
fn exprion_reports_solution_sets_cannot_participate_in_arithmetic() {
    let temp_dir = write_temp_crate(
        r#"use exprion_compile::exprion;
