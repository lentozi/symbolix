...
#[test]
fn exprion_supports_if_blocks_and_tuple_returns() {
    let compiled = exprion! {
        let x = var!("x", f64);
        let y = var!("y", f64);
...
#[test]
fn exprion_supports_boolean_variable_conditions() {
    let compiled = exprion! {
...
#[test]
fn exprion_supports_expr_macro_and_relation_methods() {
    let compiled = exprion! {
        let x = var!("x", f64);
        let rhs = expr!("x + 1");
        x.less_than(rhs)
...
#[test]
fn exprion_can_return_boolean_and_numeric_tuple_without_free_vars() {
    let compiled = exprion! {
        let x = var!("x", f64);
        let zero = expr!("0");
...
#[test]
fn exprion_solve_without_explicit_target_infers_single_variable() {
    let compiled = exprion! {
        let x = var!("x", f64);
        let lhs = expr!("2 * x + 4");
        let rhs = expr!("0");
