...
fn make_temp_dir(name: &str) -> PathBuf {
    let unique = SystemTime::now()
        ...
        .as_nanos();
    std::env::temp_dir().join(format!("symbolix_compile_{name}_{unique}"))
}
...
fn write_temp_formula_crate(body: &str) -> (PathBuf, PathBuf) {
    ...
    fs::write(
        ...
        format!(
            r#"[package]
name = "symbolix-compile-cache-case"
version = "0.1.0"
edition = "2021"
...
fn formula_uses_project_specific_cache_directory_when_configured() {
    let (temp_dir, crate_dir) = write_temp_formula_crate(
        r#"use symbolix_compile::formula;
...
"#,
    ...
    let output = Command::new("cargo")
        ...
        .env("SYMBOLIX_CACHE_DIR", &cache_dir)
        .env("SYMBOLIX_SILENCE_CACHE_WARNING", "1")
        .output()
